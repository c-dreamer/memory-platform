...
pub struct EmbeddingConfig {
    ...
    pub model: String,
    /// Expected output dimension. Vectors of any other length are rejected
    /// (fail closed) — never padded, truncated, or replaced with zeros.
    pub expected_dim: usize,
    /// NVIDIA NIM API URL.
    pub nvidia_api_url: Option<String>,
    /// NVIDIA API key.
    pub nvidia_api_key: Option<String>,
    /// NVIDIA embedding model name (e.g., "nvidia/llama-nemotron-embed-1b-v2").
...
impl LocalEmbedding {
    ...
    pub async fn new(cache_size: usize) -> Result<Self> {
        Self::with_expected_dim(cache_size, DEFAULT_EMBEDDING_DIM).await
    }

    /// Create a local backend with an explicit expected output dimension.
    pub async fn with_expected_dim(cache_size: usize, expected_dim: usize) -> Result<Self> {
    ...
    fn embed(&self, text: &str) -> Pin<Box<dyn Future<Output = Result<Embedding>> + Send>> {
        ...
        Box::pin(async move {
            if text.trim().is_empty() {
                return Ok(Embedding::new(vec![0.0; expected_dim]));
            }
            ...
            let embedding = embedding.unwrap_or_else(|| vec![0.0; expected_dim]);

            // Fail closed on dimension drift: never pad, truncate, or substitute
            // a zero vector for a wrong-sized result.
            if embedding.len() != expected_dim {
                anyhow::bail!(
                    "local embedding returned {} dimensions, expected {expected_dim}; \
                     refusing mismatched vector",
                    embedding.len()
                );
            }
...
impl NvidiaNimEmbedding {
    ...
    pub fn new(api_url: String, api_key: String, model: String, cache_size: usize) -> Self {
        Self::with_expected_dim(api_url, api_key, model, DEFAULT_EMBEDDING_DIM, cache_size)
    }

    /// Create a backend with an explicit expected output dimension.
    pub fn with_expected_dim(
        api_url: String,
        api_key: String,
        model: String,
        expected_dim: usize,
    ...
    fn parse_embedding_response(data: serde_json::Value, expected_dim: usize) -> Result<Embedding> {
        ...
        // Fail closed on dimension drift: never pad, truncate, or substitute a
        // zero vector for a wrong-sized response.
        if embedding.len() != expected_dim {
            anyhow::bail!(
                "NVIDIA embedding returned {} dimensions, expected {expected_dim}; \
                 refusing mismatched vector (re-embed required)",
                embedding.len()
            );
        }
    ...
    async fn embed_remote_chunk(
        ...
        Self::parse_embedding_response(data, expected_dim)
    }
    ...
    fn embed(&self, text: &str) -> Pin<Box<dyn Future<Output = Result<Embedding>> + Send + '_>> {
        ...
        Box::pin(async move {
            ...
            let embedding = if chunks.len() == 1 {
                ...
                let mut values = accumulator.unwrap_or_else(|| vec![0.0; expected_dim]);
                if count > 1 {
...
impl EmbeddingServiceFactory {
    ...
    ///
    /// Vectors are validated against `config.expected_dim`; a backend that
    /// cannot produce that dimension fails closed (never padded/truncated).
    pub async fn new(config: EmbeddingConfig) -> Result<Self> {
        ...
        match config.model.as_str() {
            "local" => {
                #[cfg(feature = "fastembed")]
                {
                    let local = LocalEmbedding::with_expected_dim(cache_size, expected_dim).await?;
                    ...
                    if let (Some(url), Some(key)) = (config.nvidia_api_url, config.nvidia_api_key) {
                        if !url.is_empty() && !key.is_empty() {
                            ...
                            let nvidia = NvidiaNimEmbedding::with_expected_dim(
                                url,
                                key,
                                model_name,
                                expected_dim,
            ...
            "nvidia" => {
                ...
                Ok(Self::Nvidia(NvidiaNimEmbedding::with_expected_dim(
                    api_url,
                    api_key,
                    config.nvidia_embedding_model.clone(),
                    expected_dim,
...
mod tests {
    ...
    async fn test_local_embedding() {
        let service = LocalEmbedding::with_expected_dim(1000, 384)
            .await
    ...
    #[tokio::test]
    async fn test_nvidia_embedding_rejects_dimension_drift() {
        // A 384-dim response must be refused when the expected dim is 2048.
        let response = serde_json::json!({
            "data": [{"embedding": vec![0.1_f64; 384]}]
        });
        let result = NvidiaNimEmbedding::parse_embedding_response(response, DEFAULT_EMBEDDING_DIM);
        assert!(result.is_err());
    }

    #[tokio::test]
    async fn test_empty_text() {
        let service = NvidiaNimEmbedding::with_expected_dim(
