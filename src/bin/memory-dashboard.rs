...
async fn main() -> Result<()> {
    ...
    let app = Router::new().route("/", get(|| async { Html(PAGE) })).route("/operations", get(|| async { Html(OPERATIONS_PAGE) })).route("/api/status", get(status)).route("/api/action/{action}", post(control)).with_state(state);
