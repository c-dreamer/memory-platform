...
async fn operations_page() -> Html<String> {
    Html(format!(r#"{OPERATIONS_PAGE}<style>.card:nth-child(3) .value{{color:#d4e958}}.card:nth-child(1) .value{{color:#ffcb70}}.card:nth-child(6) .value{{color:#ff8064}}#detail{{line-height:1.55;border-radius:10px}}</style><script>async function enrich(){{let s=await fetch('/api/status').then(r=>r.json());detail.textContent=`TRANSFER DECISION\nLocal database: ${{s.local_database_mb.toFixed(2)}} MB\nNeon database: ${{s.neon_database_mb===null?'unavailable':s.neon_database_mb.toFixed(2)+' MB'}}\nPending upload: ${{s.pending_upload_bytes}} bytes\nDocument parity: local ${{s.documents_active}}, Neon ${{s.neon_documents??'unavailable'}}\n\nAUTOMATION PATHS\n${{s.automation_paths.join('\n')}}\n\nORBSTACK CONTAINERS\n${{s.orbstack_containers.join('\n')||'No containers reported'}}`;}}enrich();setInterval(enrich,10000)</script>"#))
}
