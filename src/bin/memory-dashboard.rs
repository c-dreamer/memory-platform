...
struct StorageCatalog {
    ...
    document_sources: Vec<StorageCategory>,
    archive_bundles: i64,
    verified_archive_bundles: i64,
    archive_records: i64,
    policy: &'static str,
}
...
async fn probe_containers() -> Vec<String> {
        ...
        if let Ok(output) = output {
            if output.status.success() {
                if let Ok(stdout) = String::from_utf8(output.stdout) {
                    return stdout.lines().map(str::to_string).collect();
                }
            }
        }
    }
...
async fn status(
    ...
    let containers = probe_containers().await;
...
/// Redacted inventory for the app and the local control API. It intentionally
/// returns aggregate metadata only: raw transcripts and archived content stay
/// in PostgreSQL or encrypted bundles and are never sent to the dashboard.
async fn storage_catalog(
    State(state): State<Arc<DashboardState>>,
    headers: HeaderMap,
    Query(query): Query<AccessQuery>,
) -> Result<Json<StorageCatalog>, StatusCode> {
    authorized(&headers, &query, &state)?;
    let local_database_mb: f64 =
        sqlx::query_scalar("SELECT pg_database_size(current_database())::float8 / 1024 / 1024")
            .fetch_one(&state.local)
    ...
    .await
    .map_err(|_| StatusCode::SERVICE_UNAVAILABLE)?;
    let categories: Vec<StorageCategory> = sqlx::query_as(
        "SELECT category, storage_tier, count(*)::bigint, coalesce(sum(bytes),0)::bigint FROM ( \
    ...
    .map_err(|_| StatusCode::SERVICE_UNAVAILABLE)?
    .into_iter()
    .map(|(category, tier, records, bytes)| StorageCategory { category, tier, records, bytes })
    .collect();
    let document_sources: Vec<StorageCategory> = sqlx::query_as(
        "SELECT CASE \
          WHEN path LIKE 'codex://%' OR coalesce(frontmatter->>'source','') LIKE 'codex%' THEN 'Codex' \
          WHEN path LIKE 'config://opencode%' OR path LIKE 'log://opencode%' OR coalesce(frontmatter->>'source','') LIKE 'opencode%' THEN 'OpenCode' \
          WHEN path LIKE 'vault://%' OR path LIKE 'obsidian://%' OR coalesce(frontmatter->>'source','') LIKE 'vault%' THEN 'Vault' \
    ...
    .map(|(category, tier, records, bytes)| StorageCategory { category, tier, records, bytes })
    .collect();
    let (archive_bundles, verified_archive_bundles, archive_records): (i64, i64, i64) =
        sqlx::query_as(
        ...
        .await
        .map_err(|_| StatusCode::SERVICE_UNAVAILABLE)?;
    Ok(Json(StorageCatalog {
        local_database_mb,
        pending_upload_bytes,
        categories,
        document_sources,
        archive_bundles,
        verified_archive_bundles,
        archive_records,
        policy: "Active records are eligible for Neon. Archive candidates remain local until an encrypted bundle is verified; this catalog never exposes raw content.",
    }))
}
...
</script><script>async function loadCatalog(){try{let r=await fetch('/api/storage-catalog',{headers:auth});if(!r.ok)throw new Error('Storage catalog unavailable');let c=await r.json();catalog.innerHTML=c.document_sources.map(x=>`<tr><td>${x.category} / ${x.tier}</td><td>${fmt(x.records)}</td><td>${mb(x.bytes/1024/1024)}</td></tr>`).join('');archive.textContent=`Local database: ${mb(c.local_database_mb)}\nPending transfer payload: ${fmt(c.pending_upload_bytes)} bytes\n\nArchive bundles: ${fmt(c.archive_bundles)}\nVerified bundles: ${fmt(c.verified_archive_bundles)}\nArchived records: ${fmt(c.archive_records)}\n\n${c.policy}`;}catch(e){catalog.innerHTML='<tr><td colspan="3">Catalog unavailable</td></tr>';archive.textContent=e.message}}async function loadRecent(){try{let r=await fetch('/api/status',{headers:auth});if(!r.ok)return;let s=await r.json();recent.textContent=(s.recent_runs.length?s.recent_runs.map(x=>`${x.status.toUpperCase()} | ${x.started_at}${x.finished_at?' -> '+x.finished_at:''}\nrows: ${fmt(x.rows)} | bytes: ${fmt(x.bytes)}${x.error?'\nerror: '+x.error:''}`).join('\n\n'):'No Neon run records yet.')}catch(_){}}loadCatalog();loadRecent();setInterval(loadCatalog,10000);setInterval(loadRecent,10000);</script></body></html>"#;
...
async fn main() -> Result<()> {
    ...
    let app = Router::new()
        ...
        .route("/api/storage-catalog", get(storage_catalog))
        // Stable local-control API aliases. The process is loopback-only and
        // token-authenticated; VPS exposure must use a private overlay.
        .route("/health", get(status))
        .route("/storage/catalog", get(storage_catalog))
